...
class PDE_basis(object): 
    ...
    def eval_basis(self, x_in, eval_list=('u',)):
        ...
        if self.include_const:
            ...
            weight = self.weight
            bias = self.bias
        #weight = torch.tensor(weight, dtype=torch.float64).to('cuda')
        #bias = torch.tensor(bias, dtype=torch.float64).to('cuda')
        #x_in = torch.tensor(x_in, dtype=torch.float64).to('cuda')
    ...
    @staticmethod
    def nonlinear_cos(x_in, order=0):
        if order == 0:
            z_0 = np.cos(x_in)
            return [z_0]
        elif order == 1:
...
class LS_Base(object):
    ...
    @staticmethod
    def ls_fit(data, target_constrain, weights=None, ls_mse=True, item_mse=False):
        ...
        for key, value in data.items():
            ...
            if key == 'constrain':
                target_all.append((value[1] + target_constrain)*w )
            else:
    ...
    def get_mse_problem(x_in, basis, problem, coef_sol):
        ...
        for i in range(num_var):
            ...
            mse, l2, info = LS_Base.get_mse_data(x_in=x_in, y_true=target, basis=basis, coef=coef)
            fitted = info['fitted']
...
class TrainLS(LS_Base):
    ...
    def get_feature_initial(self, x_pde, x_bd, x_ic, u_old, current_sol=None):
        ...
        for key, value in bd_feature.items():
            feature_ini[key] = value
        # initial features
        ic_feature = self.problem.ls_feature_initial(x_ic, basis=self.basis)
        for key, value in ic_feature.items():
            feature_ini[key] = value
        return feature_ini
    ...
    def get_feature_others(self, x_pde, x_bd, u_old, current_sol=None):
        ...
        # boundary features
        value_feature = self.problem.ls_feature_boundary(x_bd, u_old, basis=self.basis, current_sol=current_sol)
        for key, value in value_feature.items():
            feature_all[key] = value
        return feature_all


    def get_mse_test(self, x_in, target_in, coef_sol):
        # ceof: dict of out_var coef
        basis = self.basis
        ...
        for i in range(num_var):
            ...
            mse, l2, info = LS_Base.get_mse_data(x_in=x_in, y_true=target_in, basis=basis, coef=coef)
            fitted = info['fitted']
            ...
            info1[f'{out_var}_l2'] = l2
            info1[f'{out_var}_fitted'] = fitted

        mse_all = mse_all/num_var
        l2_all = l2_all/num_var
        return mse_all, l2_all, info1
